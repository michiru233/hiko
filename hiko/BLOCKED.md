...
1.75.0（歌词页行间空隙可点回唱片）：发版记录——pubspec 1.75.0+84，`hiko-v1.75.0-android.apk` 65.8MB（aapt2 校验 versionCode='84' versionName='1.75.0'）、`hiko-v1.75.0-macos.zip` 32,641,026B，GitHub Release v1.75.0 附双资产。终验 `flutter test` 272 passed/2 skipped/0 failed（基线 270/2/0，净增 2 条）、`flutter analyze` 改动文件 0 error。模拟器端到端（深色主题、真实 vtt 歌词专辑）：暂停后点行间空隙回唱片层 ✓、点歌词文字不翻页 ✓。**排障发现（非缺陷，使用提示）**：播放中自动滚动会移动歌词列表，行间空隙随高亮句变化而位移，快速连点时目标可能移走——这是 1.61.0 自动跟随的预期行为。本版无新增待裁决；既有 1.42.0 深色主题 tag 对比度、1.53.0/1.54.0 各两项与 Mimosa 历史 high 遗留保持不变。

1.76.0（全屏歌词页点某句跳播）：发版记录——pubspec 1.76.0+85，`hiko-v1.76.0-android.apk` 65.8MB（aapt2 校验 versionCode='85' versionName='1.76.0'）、`hiko-v1.76.0-macos.zip` 32,641,029B，GitHub Release v1.76.0 附双资产。终验 `flutter test` 274 passed/2 skipped/0 failed（基线 272/2/0，净增 2 条）、`flutter analyze` 改动文件 0 error。模拟器端到端（深色主题、真实 vtt 歌词专辑）：暂停 4:13 点「本来我能加入这家公司……」→ 进度跳 4:20、被点句高亮居中、不翻页 ✓；1.75.0 空隙回唱片与点文字语义均保持 ✓。1.73.0 留下的「全屏页是否跟进点某句跳播」待裁决项就此关闭。本版无新增待裁决；既有 1.42.0 深色主题 tag 对比度、1.53.0/1.54.0 各两项与 Mimosa 历史 high 遗留保持不变。
