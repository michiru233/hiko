...
**当前版本**：v1.96.0 | [下载](https://github.com/michiru233/hiko/releases/latest)
...
- **详情抽屉**：封面、标签、RJ 号、曲目列表、完成进度、收藏、从头播放（1.50：操作按钮行改为自动换行，窄窗口下任意按钮/标签不再被横向裁剪，抽屉宽度随窗口收缩封顶 390px；1.87：声优串按顿号/逗号/斜杠/反斜杠/竖线/中点拆成逐人胶囊，灰字概览行与卡片胶囊统一显示为顿号）
...
- **进入全屏播放页的转场**（1.88）：从底部 8px 升起淡入（250ms），被覆盖的页面在 150ms 内快速退场（淡出 + 微缩 + 模糊）；返回 200ms 且位移减半。唱片在页面到位后才起转；系统「减弱动态效果」开启时自动关闭全部转场
...
- **在线（Kikoeru 服务器，1.90 起）**：接入 asmr.one 或自建 Kikoeru 实例；侧栏「在线」视图支持热门/最新/搜索三个入口、按标签翻查与排序、只看带字幕；卡片显示作品标签且**点标签即按该标签筛选**（1.94），**标签黑名单**在服务端侧过滤（1.95）；可登录账号把作品收进自己的歌单，侧栏「在线收藏」独立一页（1.93）；标签字号、卡片/详情文字倍率与每行卡片数均可调（1.96）；边播边缓存（桌面 5 GB / 移动 2 GB，可调档）；在线数据与本地库完全隔离，不写进 `library.json`
...
- 设置：分八类二级页（外观 / 音频与增益 / 主界面 / 在线账号 / 在线外观 / 数据 / 音乐目录 / 关于）；内容含主题/强调色/自定义背景图/字号/音量与增益/倍速/播放模式/侧栏/刮削代理/排序/每行专辑数（移动端独立档位）/快进秒数/评分/歌词字号/在线服务器与缓存/在线标签字号与每行卡片数
