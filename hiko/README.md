...
**当前版本**：v1.86.0 | [下载](https://github.com/michiru233/hiko/releases/latest)
...
- **全屏播放页歌词**（1.61–1.76）：中央区域唱片/歌词双层切换——点唱片进歌词、点行间空隙或上下留白回唱片、点某句歌词跳播到该句并立即居中、无歌词时点任意处返回；歌词自动跟随高亮句滚动（手动滑走暂停 3 秒后自愈，右下角「回到当前句」按钮兜底），字号 5 档可调；首末句也能滚到正中
...
- **键盘快捷键**（桌面，1.86 起全局生效——详情页/全屏播放页/对话框打开时同样可用）：空格播放/暂停、←→ 快退/快进（步长 3/5/10/15/30/60 秒可调，默认 3）、↑↓ 上一首/下一首、Esc 逐层返回上一级；输入框聚焦时自动让位给打字；⌘K/Ctrl+K 聚焦搜索、⌘O/Ctrl+O 导入、⌘⇧H/Ctrl+Shift+H 隐私模糊
- **自定义背景图**（1.84）：任选图片铺根层底，模糊度与不透明度可调，浅/深主题自动叠可读性薄纱；全平台生效，未启用零开销
...
产物：`dist/Hiko-<version>.dmg`（dmg 路线）、`hiko/hiko-v<version>-macos.zip` 与 `hiko/hiko-v<version>-android.apk`（GitHub Release 路线，落在 `hiko/` 并已 gitignore）。
...
- 设置：分六类二级页（外观 / 音频与增益 / 主界面 / 数据 / 音乐目录 / 关于）；内容含主题/强调色/自定义背景图/字号/音量与增益/倍速/播放模式/侧栏/刮削代理/排序/每行专辑数（移动端独立档位）/快进秒数/评分/歌词字号
