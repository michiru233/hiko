...
经过全面代码审查，发现**原计划中的大部分功能已经是跨平台实现，macOS 早已可用**：

**第一批（播放体验）- 无需同步**
- ✅ 睡眠定时器（1.29.0）- 已跨平台
- ✅ 播放倍速（1.29.0）- 已跨平台
- ✅ 进度记忆 + 继续收听横幅（1.41.0）- 已跨平台
...
**第三批（UI 优化）- 无需同步**
- ✅ 隐私模糊（1.52.0）- 已跨平台（含 macOS 特殊处理）
- ✅ 瀑布流 + 动态卡片（1.51.0）- 已跨平台
- ✅ 字号全局缩放（1.56.0）- 已跨平台
...
实现位置：`lib/data/scanner.dart`
- 新增常量：`lyricExtensions`、`lyricMaxBytes`（64KB）
- 新增函数：`_findLyricFor()` 查找同名歌词文件
- 新增函数：`_readLyricText()` 读取并多编码解码（复用 `repairText`）
- 修改：`_buildAlbum()` 中为每个 Track 自动关联歌词

对齐 Android 行为：
- 同名匹配（大小写不敏感）：`01.mp3` → `01.lrc/01.vtt/01.srt`
- 文件大小限制：≤64KB
- 多编码解码：UTF-8 / Shift-JIS / GBK（CJK 评分）

测试结果：
- 237 个单元测试通过
- macOS Release 构建成功（31MB）

发布：
- Git 提交：`450f46e`
- GitHub Release：[v1.62.0](https://github.com/michiru233/hiko/releases/tag/v1.62.0)
- 资产：`hiko-v1.62.0-macos.zip`（31MB）

### 结论

**原计划的"Android → macOS 同步"任务实际上大部分已完成**，Flutter 跨平台架构使得功能天然在 macOS 上可用。本次仅补充了歌词自动检测这一 Android 特有逻辑的 macOS 适配。

**下一步建议**：
- 第三批 UI 优化功能已全部存在，无需额外工作
- 音频增益扩展（1.0~4.0x）仍为单独立项，需技术验证后实施
