...
- **根因（1.73.0 设计的副作用）**：`_buildLyricsView` itemBuilder 里每行歌词**整行**（含 `vertical: 8` 间距）都包在一层故意吸收点击的 `GestureDetector` 里——这是 1.73.0「点歌词文字不翻页」裁决的实现方式。密歌词时（用户截图整屏都是行区域）几乎不存在可点的空白，真正能触发回唱片手势的只剩首句之上／末句之下两条窄边，还要躲开自动滚动导致的列表位移。
- **用户裁决（grilling）**：①保留「点歌词文字不翻页」，把吸收点击的范围收窄到**文字渲染框**（Text 的整个 box，含自动换行的多行整块），行与行之间的空隙落回外层翻页手势（否决「点任意处都回唱片」与「点句子跳播」）；②验证走测试 + 模拟器实测。
- **修复（`fullscreen_player_screen.dart` itemBuilder 一处，GestureDetector 与 Padding 交换嵌套顺序）**：`Padding(vertical:8)` 在外、吸收手势在内只包 `Text`。外层手势、LayoutBuilder、浮层按钮均未动；`drawer_lyrics_view.dart` 未触碰。
- **回归测试（新增 2 条，`test/ui/lyrics_gap_tap_back_test.dart`）**：①点相邻两行歌词文字框之间的空隙（16 逻辑 px）回唱片层；②点歌词文字仍不翻页（守住收窄没过头）。**测试基建坑**：歌词解析依赖播放状态更新事件，settle 时必须像旧 toggle 测试那样每帧刷新 `playbackProvider`，否则歌词页落在「暂无歌词」分支、`find.text('第 0 句')` 找不到（首跑两条全红的原因，dump 后定位）。
- **反向验证**：`git checkout` 还原旧嵌套 → 仅「空隙回唱片」转红（+1 -1，「文字不翻页」保持绿），重新 `git apply` 修复 patch → 2/2 全绿。
- **端到端（kikoeru_test 模拟器，深色主题，真实专辑 RJ01414585 带 vtt 歌词，SAF 导入）**：播放中点行间空隙首次未翻页——**播放中自动滚动会移动列表，固定坐标瞄准失效**（排障记录）；暂停播放冻结列表后点行间空隙 → 成功回唱片层 ✓；再进歌词页点歌词文字正中 → 仍停歌词页 ✓。截图存 `.shots/hiko-175-*.png`。
- **验证**：`flutter test` **272 passed / 2 skipped / 0 failed**（1.74.0 基线 270/2/0，净增 2 条）；`flutter analyze` 改动文件 0 error（4 条 info 为历史遗留 deprecation）。
- **构建产物**：Android apk 65.8MB（aapt2 校验 `versionCode='84' versionName='1.75.0'`）；macOS Hiko.app 73.8MB → `hiko-v1.75.0-macos.zip` 32,641,026B（`unzip -t` OK）。
- **版本记录**：1.74.0+83 → **1.75.0+84**。
...
- **决策过程**：按 grilling 走一轮设计树评审，产出决策书 `.zcode/plans/decision-hiko-view-toggle.md`（含全部实测约束与选项权衡；当时另写的 goal 任务书已在 1.86.0 洁癖收尾中按「结论已并入本节」删除）。四条用户裁决：①歌词页**只认空白处**（点歌词文字不翻页）；②唱片页**整个中央区域**都可点；③切到歌词页**总是定位到当前句**；④加 **220ms 交叉淡入淡出**。
- **实测约束（先取证再定方案）**：
...
- **同批收尾（用户拍板「那改吧」）**：把 `update_checker_network_test.dart` 改为**默认跳过、按需启用**（照 `dlsite_scraper_network_test.dart` 的既有惯例），`flutter test` 从此不依赖外部 API 额度。裸跑在匿名额度 `remaining=0` 时依然全绿：**268 passed / 2 skipped / 0 failed**（跳过数由 1 升 2 是本改动的目的本身——把依赖外部额度的实网用例移出默认路径，不是藏失败）。三条反向验证：①裸跑 → `All tests skipped`；②门控打开 + 真 token → 真跑实网并通过；③**门控打开 + 故意传无效 token → `HttpException: GitHub API 401`**，证明门控打开后请求确实发出、断言确实执行，跳过逻辑掩盖不了坏掉的 API 路径。限流的实测成因：GitHub 匿名额度是「60 次/小时/**出口 IP**」且与出口上的其他使用者共享；本机全部 GitHub 流量走本地代理，实测出口 `203.27.106.243`（新加坡），且 `--noproxy '*'` 出口 IP 不变（代理在网卡层接管），故那 60 个号不归本机独占。纯测试基础设施改动，不影响已发布产物，故不 bump 版本、不发新 Release。
...
- **决策过程**：先按 grilling 走了一轮设计树评审，产出决策书 `.zcode/plans/decision-hiko-lyrics-center.md`（含全部实测数字与选项权衡；当时另写的 goal 任务书已在 1.86.0 洁癖收尾中按「结论已并入本节」删除）。三条用户裁决：①加首尾留白让首末句也能居中；②修根因＋全屏页也加按钮；③拖完进度条松手即恢复跟随并立即居中。
- **三个根因（全部实测取证，非推断）**：
...
- **验证**：`flutter test` 245 passed / 1 skipped / 1 failed（唯一失败为 `update_checker_network_test` 断言最新 Release 含 macos 资产——本次补 macos 资产后自愈）。**用户真机确认已解决**。
- **构建产物**：Android `flutter build apk --release` → 65.7MB `hiko-v1.71.1-android.apk`；macOS `flutter build macos --release` → 31MB `hiko-v1.71.1-macos.zip`。
- **版本记录**：1.70.0+78 → 1.71.1+80（1.71.0 为误诊版本，已作废）；commit `1a9e454` 已推送 origin main；GitHub Release [v1.71.1](https://github.com/michiru233/hiko/releases/tag/v1.71.1) 双资产。
...
### 1.86.0 洁癖收尾（2026-09-14）

事实面状态：代码/运行态 `verified-current`（Release v1.86.0 双资产非 draft，`fe9f998` 已推送）；文档 `changed-and-verified`；规则 `changed-and-verified`；记忆 `not-applicable`（本平台无受控记忆入口）；工作区 `changed-and-verified`。

改动：
- `README.md`（根，用户门面）：现役版本指针 1.54.1 → 1.86.0；键盘控制条目补 15/60 秒档位、Esc 逐层返回、全局生效范围（详情页/全屏播放页/弹窗）；外观区补「自定义背景图」与「设置六类二级页」两条。
- `hiko/README.md`：版本行 1.76.0 → 1.86.0；键盘快捷键条目同步（全局生效 + 15/60 档 + Esc）；新增「自定义背景图」条目；数据区设置清单补分类导航与新增项。
- `AGENTS.md`：修正 `hiko/PROGRESS.md`「止于 1.39.0」→ 实际 1.78.0；补充「1.79 起各版待裁决状态以 plan 文件为准」；补「apk 资产不入 git」「`.shots/` 调试截图不入库」。
- `hiko/BLOCKED.md`：补记 1.79.0–1.85.0 期内无新增待裁决 + 1.86.0 记录，并声明此后不再逐版追加流水账（避免与 plan 文件双份真相）。
- `.gitignore`：新增 `.shots/`；`git rm -r --cached .shots` 撤出 1.86.0 提交被 `git add -A` 误收的 44 张调试截图（磁盘文件保留 44 个，未删除）。

既有待裁决（未消除，非本次范围）：1.42.0 深色主题 tag 对比度；1.53.0 两项（Android 详情页「整理专辑」入口语义不成立、多行 TALB 参与分组键）；1.54.0 两项（左缘右滑受系统手势排除区限制、同名文件原位替换不触发增量重扫）；Mimosa 历史 high 12 项（1.48.0 未裁决）。本版无新增。

### 1.86.0 清场结果（用户 2026-09-14 汇报后确认「删」）

删除前逐个核对 GitHub Releases：v1.71.1–v1.86.0 全部存在且资产齐全（唯 v1.77.0 本就只有 android 资产，与磁盘一致，无独有副本）；v1.37.0/v1.38.0/v1.50.0 三个 dist zip 对应的 Release 也存在。据此执行：

- `hiko/*.zip` + `hiko/*.apk`（31 个文件，1.4GB）与 `hiko/dist/`（3 个旧 zip，76MB）已删除——均为 GitHub Releases 已有的历史构建产物，释放约 1.5GB。
- `.shots/` 44 张一次性调试截图整体删除。plan 中 1.74.0/1.75.0 章节的「截图存 `.shots/hiko-17x-*.png`」为当时记录，对应文件已不存在。
- `.zcode/plans/goal-hiko-lyrics-center.md`、`goal-hiko-view-toggle.md` 两份一次性任务书删除。
...
## 1.97.2 筛选标记挪出第二行：移动端独占一行（2026-09-26）

1.97.1 的修复只解决了标记**内部**的收缩，实机截图（1.97.0 用户第二张反馈）显示
标记仍被截断 —— **根因升级**：重构时外层 `Flexible` 一并丢了，而第二行在手机竖屏上
本来只剩一百多像素给尾部，塞什么都截断。用户裁决：「放到别的地方」。

- **移动端**：激活的筛选标记（标签 / 声优 / 社团，可同时多个）从第二行挪出，
  **独占一行**（第二行下方），左对齐、整行可用；`maxTextWidth` 放宽到 320，
  长名字能显示完整。第二行剩「排序 + Aa + 黑名单标记 + 状态行」，状态行恢复呼吸空间。
- **桌面**：保持内联不动（用户：不涉及 mac 就别动 mac）—— 但内联处补上了
  `Flexible` 包裹（1.97.1 丢失的那层），窄窗口下同样安全。
